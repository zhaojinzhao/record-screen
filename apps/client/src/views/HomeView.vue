...
let timer: any;
watch(errorAlertConfig, () => {
  if (errorAlertConfig.value.visible) {
    clearTimeout(timer);

    timer = setTimeout(() => {
      errorAlertConfig.value.visible = false;
    }, 3000);
  }
});
...
const previewVideoBlob = (previewVideo: HTMLVideoElement, chunk: Blob) => {
  previewVideo.src = URL.createObjectURL(chunk as Blob);
  previewVideo.play();
};
...
const previewVideo = (chunk: Blob) => {
  const previewVideoEl = previewVideoRef.value;

  if (!(previewVideoEl && chunk)) return;

  previewVideoEl.srcObject = null;

  previewVideoBlob(previewVideoEl, chunk);
};

const onStopRecording = (chunk: Blob) => {
...
const startRecording = async () => {
  try {
    ...
    if (!(previewVideoEl && stream)) return;

    // 预览屏幕流
    previewScreenStream(previewVideoEl, stream);
  ...
  <div class="record-screen">
    ...
    >
      <a-alert
        class="error-alert"
        type="error"
        v-show="errorAlertConfig.visible"
      >
        {{ errorAlertConfig.msg }}
      </a-alert>
    </Transition>
    <video
      controls
    ...
    <div class="controls">
      <div
        class="control-btn stop-recording-btn"
        @click="stopRecording"
        v-if="isRecording"
      >
        完成录制
      ...
      <template v-else>
        <template v-if="recorder">
          <div class="control-btn start-recording-btn" @click="startRecording">
            重新录制
          </div>
          <div class="control-btn download-video-btn" @click="downloadVideo">
            下载视频
          </div>
        </template>
        <div
          v-else
          class="control-btn start-recording-btn"
